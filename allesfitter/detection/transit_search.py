...
def get_tls_kwargs_by_tic(tic_id, sigma=3, tls_kwargs=None, quiet=True):
    ...
    if np.isnan(tls_kwargs['R_star']):
        tls_kwargs['R_star'] = 1.
        warnings.warn(
            "tls_kwargs: R_star was undefined in TICv8. Filling it with R_star=1.")
    ...
    if np.isnan(tls_kwargs['M_star']):
        tls_kwargs['M_star'] = 1.
        warnings.warn(
            "tls_kwargs: M_star was undefined in TICv8. Filling it with M_star=1.")
    ...
    if np.isnan(tls_kwargs['u']).any():
        ...
        warnings.warn(
            "tls_kwargs: u was undefined in TICv8. Filling it with u=[0.4804, 0.1867]")
...
def tls_search(time, flux, flux_err, plot=True, plot_type='brokenplot', **kwargs):
    ...
    **kwargs : collection of keyword arguments
    ...
    non_tls_keys = ['SNR_threshold', 'SDE_threshold',
                    'FAP_threshold', 'quiet', 'inj_period']
    tls_kwargs_original = {key: kwargs[key] for key in kwargs.keys(
    ) if key not in non_tls_keys}  # for the original tls
    ...
    def _run1(time, flux, flux_err):
        ...
        results['detection'] = (results['snr'] >= kwargs['SNR_threshold']) and (
            results['SDE'] >= kwargs['SDE_threshold']) and (results['FAP'] <= kwargs['FAP_threshold'])
        ...
        if results['detection']:
            ...
            if 'R_star' in kwargs:
                results['R_planet'] = results['rp_rs'] * kwargs['R_star'] * \
                    109.07637070600963  # from Rsun to Rearth
    ...
    while (SNR >= kwargs['SNR_threshold']) and (SDE >= kwargs['SDE_threshold']) and (FAP <= kwargs['FAP_threshold']) and (FOUND_SIGNAL == False):
        ...
        if results['detection']:
            ...
            if plot_bool:
                ...
                fig_lightcurve = _tls_search_plot_lightcurve(
                    time, flux, results, typ=plot_type)
...
def _tls_search_plot_lightcurve(time, flux, results, typ='fullplot'):
    ...
    if typ == 'fullplot':
        ...
        axes = fullplot(results['model_lightcurve_time'], results['model_lightcurve_model'],
                        color='r', ls='-', marker='', lw=3, zorder=100, axes=axes)
    elif typ == 'brokenplot':
        axes = brokenplot(time, flux)
        axes = brokenplot(results['model_lightcurve_time'], results['model_lightcurve_model'],
                          color='r', ls='-', marker='', lw=3, zorder=100, axes=axes)
    elif typ == 'tessplot':
        trend = _cut(time, results['model_lightcurve_time'],
        ...
        axes = monthplot(results['model_lightcurve_time'], results['model_lightcurve_model'],
                         color='r', ls='-', marker='', lw=3, zorder=100, axes=axes)
...
def _tls_search_plot_folded(time, flux, results):
    ...
    bintime, binflux, binflux_err, _ = rebin_err(
        results['folded_phase'], results['folded_y'], dt=0.001*results['period'], ferr_type='medsig', ferr_style='sem')
    ax.plot(bintime, binflux, 'b.', rasterized=True)
    ax.plot(results['model_folded_phase'],
            results['model_folded_model'], 'r-', lw=3)
    ylim = ax.get_ylim()
    ax.plot(results['folded_phase'], results['folded_y'],
            'k.', color='silver', rasterized=True, zorder=-1)
    ax.set_ylim(ylim)

    ax = axes[1]
    bintime, binflux, binflux_err, _ = rebin_err(
        (results['folded_phase']-0.5)*results['period']*24, results['folded_y'], dt=0.001*results['period']*24, ferr_type='medsig', ferr_style='sem')
    ax.plot(bintime, binflux, 'bo', rasterized=True)
    ax.plot((results['model_folded_phase']-0.5)*results['period']
            * 24, results['model_folded_model'], 'r-', lw=3)
    ax.set(xlim=[-1.5*results['correct_duration']*24, +1.5 *
                 results['correct_duration']*24], xlabel='Time (h)', yticks=[])
    ylim = ax.get_ylim()
    ax.plot((results['folded_phase']-0.5)*results['period']*24,
            results['folded_y'], 'k.', color='silver', rasterized=True, zorder=-1)
    ax.set_ylim(ylim)

    ax = axes[2]
    ax.text(.02, 0.95, 'P = ' + np.format_float_positional(
        results['period'], 4) + ' d', ha='left', va='center', transform=ax.transAxes)
    ax.text(.02, 0.85, 'Depth = ' + np.format_float_positional(1e3*(1. -
                                                                    results['depth']), 4) + ' ppt', ha='left', va='center', transform=ax.transAxes)
    ax.text(.02, 0.75, 'Duration = ' + np.format_float_positional(24 *
                                                                  results['correct_duration'], 4) + ' h', ha='left', va='center', transform=ax.transAxes)
    ax.text(.02, 0.65, 'T_0 = ' + np.format_float_positional(
        results['T0'], 4) + ' d', ha='left', va='center', transform=ax.transAxes)
    ax.text(.02, 0.55, 'SNR = ' + np.format_float_positional(
        results['snr'], 4), ha='left', va='center', transform=ax.transAxes)
    ax.text(.02, 0.45, 'SDE = ' + np.format_float_positional(
        results['SDE'], 4), ha='left', va='center', transform=ax.transAxes)
    ax.text(.02, 0.35, 'FAP = ' + np.format_float_scientific(
        results['FAP'], 4), ha='left', va='center', transform=ax.transAxes)
    ax.text(.02, 0.25, 'R_planet/R_star = ' + np.format_float_positional(
        results['rp_rs'], 4), ha='left', va='center', transform=ax.transAxes)
    if ~np.isnan(results['R_planet']):
        ax.text(.02, 0.15, 'R_planet = ' + np.format_float_positional(
            results['R_planet'], 4), ha='left', va='center', transform=ax.transAxes)
    ax.set_axis_off()
...
def tls_search_tess(time, flux, flux_err,
    ...
    write_csv(os.path.join(options['outdir'], 'flux_clean.csv'),
              (time, flux_clean, flux_err), header='time,flux_clean,flux_err')
    ...
    if len(results_all) > 0:
        ...
        for i, results in enumerate(results_all):
            write_tls_results(os.path.join(
                options['outdir'], 'tls_signal_'+str(i)+'.txt'), results)
...
def tls_search_by_tic(tic_id,
    ...
    time, flux, flux_err = tessio.get(
        tic_id, pipeline='spoc', PDC=True, unpack=True)
